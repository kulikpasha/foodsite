...
class RecipesController {
    async create(req, res, next){
        try {
            ...
            const recipe = await Recipe.create({ name, img: fileName, typeId, brandId})
    ...
    async getAll(req, res){
        ...
        if (!brandId && !typeId) {
            ...
            recipes = await Recipe.findAndCountAll({where:{brandId},limit, offset})
        }
        else if (!brandId && typeId) {
            recipes = await Recipe.findAndCountAll({where:{typeId},limit, offset})
        }
        else if (brandId && typeId) {
            recipes = await Recipe.findAndCountAll({where:{typeId, brandId},limit, offset})
        }
        ...
        const recipe = await Recipe.findOne(
            {
                ...
                include: [
                    ...
                    {model: Ingridient, as:'ingridients'}
                ]
